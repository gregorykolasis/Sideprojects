...
class myRS485Programmer(asyncio.Protocol):
    ...
    async def send_firmware_chunk(self, slave_address, current_chunk_num, chunk, chunk_num, total_chunks):
        ...
        #print(f"[DEBUG] Sending chunk bytes (DEC): {data_str}")     
        ...
        while True:
            ...
            sys.__stdout__.write(
                f"\r[PROGRESS] Chunk {chunk_num}/{total_chunks} "
    ...
    async def update_firmware(self,filePath=FIRMWARE_PATH,slave_address=1):
        ...
        await self.send_start_command(slave_address=slave_address, firmware_size=firmware_size, total_chunks=total_chunks, firmware_version=firmware_version)
        await asyncio.sleep(1)  # Give the ESP32 time to process the start command
        ...
        for i in range(0, firmware_size, CHUNK_SIZE):
            ...
            if not await self.send_firmware_chunk(slave_address=slave_address, current_chunk_num=current_chunk_num, chunk=chunk, chunk_num=(i//CHUNK_SIZE)+1, total_chunks=total_chunks):
