...
def initialized():
    print("============================================pyserial-asyncio--Initialized===============================")
...
class SerialTransport(asyncio.Transport):
    ...
    def __init__(self, loop, usb_lost , protocol, serial_instance):
        super().__init__()
        self._loop = loop
        ...
        self.serial_disconnection_callback = usb_lost
        #initialized()
    ...
    def set_serial_connection_callback(self , func):
        self.serial_disconnection_callback = func
        ...
        def _poll_read(self):
            if self._has_reader and not self._closing:
                try:
                    ...
                    self._fatal_error(exc, 'Fatal write error on serial transport')
                    self.serial_disconnection_callback()
...
async def create_serial_connection(loop, usb_lost , protocol_factory, *args, **kwargs):
    ...
    transport, protocol = await connection_for_serial(loop, usb_lost , protocol_factory, serial_instance)
    return transport, protocol


async def connection_for_serial(loop, usb_lost , protocol_factory, serial_instance):
    ...
    transport = SerialTransport(loop, usb_lost , protocol, serial_instance)
    return transport, protocol
