...
class Programmer( mySerial , myKeyboard ):
    ...
    def createProgrammingCmd(self):
        ...
        avrdude = f"{path}/tools/apps/1.8.9/{self.platform}/bin/avrdude"
        conf    = f"{path}/tools/apps/1.8.9/{self.platform}/etc/avrdude.conf"

        file = f"{path}/Build/{self.buildfile}{self.buildFileExtension}"
        programmingPort = self.port

        command = f"{avrdude} -C{conf} -v -p{board} -cwiring -P{programmingPort} -b115200 -D -Uflash:w:{file}:i"
