...
class Programmer( mySerial , myKeyboard ):
    ...
    def createProgrammingCmd(self,UF2_BAORD = False):
        command = ''
        tool = f"{path}/tools/apps/uf2conv.py"
        file = f"{path}/Build/{self.buildfile}.ino.uf2"
        programmingPort = 'Unknown'
        if UF2_BAORD: programmingPort = "UF2_BOARD"
        else: programmingPort = self.port
        buildCmd = f"{tool} --serial {programmingPort} --family RP2040 --deploy {file}"
        if self.platform == "windows" : command = f"python {buildCmd}" 
        if self.platform == "linux"   : command = f"sudo python3 {buildCmd}" 
        return command
    ...
    async def startProgrammer(self,isInteractive):
        ...
        try:
            ...
            if returnCode!=0:
                self.logger.error(f"[startProgrammer] Return code:{returnCode}")    
                returnCode = os.system( self.createProgrammingCmd(True) )
            else:
                self.logger.critical(f"[startProgrammer] All good")  
